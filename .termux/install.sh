...
echo -e "${GREEN}Time to install Nala Package Manager, Termux Clipboard, Neovim, NodeJS, Python-pip, Perl, Ruby, LuaRocks, LuaJIT, LazyGit, wget, gettext, logo-ls, ncurses-utils, libuv, Timewarrior, Taskwarrior, Zoxide and htop!${ENDCOLOR}"
sleep 5
...
nala install termux-api gh neovim nodejs python-pip perl ruby luarocks luajit ripgrep fd lazygit wget gettext logo-ls ncurses-utils libuv timewarrior taskwarrior zoxide htop -y || error_exit "${RED}Failed to install packages.${ENDCOLOR}"
