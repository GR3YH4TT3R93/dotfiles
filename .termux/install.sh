...
echo -e "${GREEN}Time to install Nala Package Manager, Termux Clipboard, Neovim, NodeJS, Python-pip, Ruby, LuaRocks, LuaJIT, wget, gettext, logo-ls, ncurses-utils, Timewarrior, Taskwarrior, and htop!${ENDCOLOR}"
sleep 5
...
nala install termux-api gh neovim nodejs python-pip ruby luarocks luajit ripgrep fd wget gettext logo-ls ncurses-utils timewarrior taskwarrior htop -y || error_exit "${RED}Failed to install packages.${ENDCOLOR}"
