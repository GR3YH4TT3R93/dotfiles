...
echo -e "${GREEN}Time to install Nala Package Manager, Termux Clipboard, Neovim, Lua Language Server, Stylua, NodeJS, Python-pip, Perl, Ruby, LuaRocks, LuaJIT, LazyGit, wget, gettext, logo-ls, ncurses-utils, libuv, Timewarrior, Taskwarrior, Zoxide and htop!${ENDCOLOR}"
sleep 5
...
nala install termux-api gh neovim lua-language-server stylua nodejs python-pip perl ruby luarocks luajit ripgrep fd lazygit wget gettext logo-ls ncurses-utils libuv timewarrior taskwarrior zoxide htop -y || error_exit "${RED}Failed to install packages.${ENDCOLOR}"
